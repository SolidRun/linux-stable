...
#include "irq-gic-rzv2m-common.h"
#include "irq-gic-rzv2m.h"
...
static int gic_set_type(struct irq_data *d, unsigned int type)
{
	...
	ret = rzv2m_gic_configure_irq(gicirq, type, base + GIC_DIST_CONFIG, NULL);
	if (ret && gicirq < 32) {
...
void __init rzv2m_gic_cascade_irq(unsigned int gic_nr, unsigned int irq)
{
...
static void gic_dist_init(struct gic_chip_data *gic)
{
	...
	t_size = sizeof(rzv2m_gic_init_target) / sizeof(u32);
	for (cnt = 0; cnt < t_size; cnt++)
		writel_relaxed(rzv2m_gic_init_target[cnt], base + GIC_DIST_TARGET + (0x20+(cnt*4)));

	rzv2m_gic_dist_config(base, gic_irqs, NULL);
...
static int gic_cpu_init(struct gic_chip_data *gic)
{
	...
	rzv2m_gic_cpu_config(dist_base, 32, NULL);
...
int rzv2m_gic_cpu_if_down(unsigned int gic_nr)
{
 ...
 */
void rzv2m_gic_dist_save(struct gic_chip_data *gic)
{
	unsigned int gic_irqs;
 ...
 */
void rzv2m_gic_dist_restore(struct gic_chip_data *gic)
{
	unsigned int gic_irqs;
	unsigned int i;
	void __iomem *dist_base;
...
void rzv2m_gic_cpu_save(struct gic_chip_data *gic)
{
	int i;
	u32 *ptr;
	void __iomem *dist_base;
...
void rzv2m_gic_cpu_restore(struct gic_chip_data *gic)
{
	int i;
	u32 *ptr;
	void __iomem *dist_base;
...
static int gic_notifier(struct notifier_block *self, unsigned long cmd,	void *v)
{
	...
	for (i = 0; i < CONFIG_ARM_GIC_MAX_NR; i++) {
		switch (cmd) {
		case CPU_PM_ENTER:
			rzv2m_gic_cpu_save(&gic_data[i]);
			break;
		case CPU_PM_ENTER_FAILED:
		case CPU_PM_EXIT:
			rzv2m_gic_cpu_restore(&gic_data[i]);
			break;
		case CPU_CLUSTER_PM_ENTER:
			rzv2m_gic_dist_save(&gic_data[i]);
			break;
		case CPU_CLUSTER_PM_ENTER_FAILED:
		case CPU_CLUSTER_PM_EXIT:
			rzv2m_gic_dist_restore(&gic_data[i]);
			break;
		}
	}
...
void __init rzv2m_gic_init(void __iomem *dist_base, void __iomem *cpu_base)
{
...
static int gic_of_setup(struct gic_chip_data *gic, struct device_node *node)
{
	...
	rzv2m_gic_enable_of_quirks(node, gic_quirks, gic);
...
int rzv2m_gic_of_init_child(struct device *dev, struct gic_chip_data **gic, int irq)
{
	int ret;
...
static void __init gic_of_setup_kvm_info(struct device_node *node)
{
	...
	if (static_branch_likely(&supports_deactivate_key))
		rzv2m_gic_set_kvm_info(&gic_v2_kvm_info);
}

int __init
rzv2m_gic_of_init(struct device_node *node, struct device_node *parent)
{
	struct gic_chip_data *gic;
	int irq, ret;
	...
	if (parent) {
		...
		rzv2m_gic_cascade_irq(gic_cnt, irq);
	}
...
IRQCHIP_DECLARE(gic_400, "arm,gic-400-rzv2m", rzv2m_gic_of_init);
