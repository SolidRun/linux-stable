...
static const struct snd_soc_dapm_widget imx_wm8904_dapm_widgets[] = {
	SND_SOC_DAPM_LINE("Line Out Jack Left", NULL),
	SND_SOC_DAPM_LINE("Line In Jack Right", NULL),
	SND_SOC_DAPM_LINE("Mic in", NULL),
	SND_SOC_DAPM_LINE("Micbias", NULL),
};
...
static int imx_wm8904_probe(struct platform_device *pdev)
{
	...
	if (!codec_pdev || !codec_pdev->dev.driver) {
		if (!codec_pdev){
			dev_err(&pdev->dev, "!codec_pdev problem\n");
		}
	...
	imx_wm8904_dai[0].playback_only	= 1;
